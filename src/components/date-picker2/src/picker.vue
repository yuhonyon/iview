<template>
  <i-input
    ...
    @on-keydown.native="handleKeydown"
    :value="displayValue"
    @on-input="value => userInput = value"
...
export default {
  ...
  computed: {
    ...
    triggerClass() {
      if(this.showClose&&this.haveTrigger){
        return this.clearIcon
      }
      return this.prefixIcon || (this.type.indexOf('time') !== -1 ? 'ios-clock-outline' : 'ios-calendar-outline');
    },
  ...
  methods: {
      ...
      const updateOptions = () => {
        ...
        for (const option in options) {
          if (options.hasOwnProperty(option) &&
              ...
              option !== 'selectableRange'&&(option!=="disabledDate"||(!maxDate&&!minDate))) {
    ...
    unmountPicker() {
      if (this.picker) {
        ...
        if (typeof this.unwatchMaxDate === 'function') {
          this.unwatchMaxDate();
        }
