...
const getI18nSettings = () => {
    return {
        dayNamesShort: weeks.map(week => t(`i.datepicker.weeks.${ week }`)),
        dayNames: weeks.map(week => t(`i.datepicker.weeks.${ week }`)),
        monthNamesShort: months.map(month => t(`i.datepicker.months.${ month }`)),
        monthNames: months.map((month, index) => t(`i.datepicker.month${ index + 1 }`)),
        amPm: ['am', 'pm']
    };
};
